...
public class TicTacToe {
    ...
    public static void main(String[] args) {
        ...
        if (is_win_by_row(arrBoard)) {
            ...
            System.out.println("row starter is : " + get_row_number_starter(arrBoard));
            System.out.println("(" + get_winner_evidence_by_row(get_row_number_starter(arrBoard), arrBoard) + ")");
        } else if (is_win_by_col(arrBoard)) {
            System.out.println("win by col is : " + is_win_by_col(arrBoard));
            System.out.println("col starter is : " + get_col_number_starter(arrBoard));
            System.out.println("(" + get_winner_evidence_by_col(get_col_number_starter(arrBoard), arrBoard) + ")");
        } else if (is_win_by_diagonal(arrBoard)){
            System.out.println("win by diagonal is : " + is_win_by_diagonal(arrBoard));
            System.out.println("diagonal starter is : " + get_diagonal_number_starter(arrBoard));
            System.out.println("(" + get_winner_evidence_by_diagonal(get_diagonal_number_starter(arrBoard), arrBoard) + ")");
        } else {
            System.out.println("something not right. contact us for help.");
        }
    ...
    public static boolean is_win_by_col(int[][] arrBoard) {
        ...
        for (int column_index = 0; column_index < arrBoard.length; column_index++) {
            ...
            for (int row_index = 0; row_index < arrBoard.length; row_index++) {
                ...
                // compare one row to next row
                // eg: row 1 and row 2,
                // if value in row 1 = value in row 2,
                // then we want to continue the loop
                if (arrBoard[row_index][column_index] != 0 && arrBoard[row_index][column_index] == arrBoard[row_index + 1][column_index]) {

                    // once reach this line, means each checking is match until the last loop,
                    // so we know now this column is match (all value match)
                    // so we declare win = true to break the row loop (don't want to check other column)
                    if (row_index == arrBoard.length - 2) {
                        win = true;
                        break;
                    }
                } else {
                    // once not match, go next column
                    break;
                }
            }
        }
    ...
    public static int get_col_number_starter(int[][] arrBoard) {

        // assign number each time enter new col (so we get number starter for that col)
        int starter_number = 0;
        // start with zero, will increment this with each loop below to keep track of where we are (matrix)
        int matrix = 0;

        // once we set this to true / we find the matched col, we want to break the
        // loop.
        boolean match = false;
        ...
        for (int column_index = 0; column_index < arrBoard.length; column_index++) {
            ...
            for (int row_index = 0; row_index < arrBoard.length; row_index++) {
                ...
                if (row_index == 0) {
                    // get matrix number starter for that column, only applicable when row index = 0,
    ...
    public static String get_winner_evidence_by_col(int column_number, int[][] arrboard) {
        String evidence = "";
        for (int i = 0; i < arrboard.length; i++) {
            if (i == 0) {
            ...
            column_number = column_number + arrboard.length;
        }
        return evidence;
    }
    ...
    public static int get_diagonal_number_starter(int[][] arrBoard) {
        ...
        // only have two posibble starter
        if (arrBoard[1][1] != 0 &&
                (arrBoard[0][0] == arrBoard[1][1] && arrBoard[1][1] == arrBoard[2][2])) {
            starter_number = 1;
        } else if (arrBoard[1][1] != 0 &&
                (arrBoard[0][2] == arrBoard[1][1] && arrBoard[1][1] == arrBoard[2][0])) {
            starter_number = 3;
        }
        return starter_number;
    ...
    public static String get_winner_evidence_by_diagonal(int starter, int[][] arrboard) {
        String evidence = "";
        int number = starter;
        for (int i = 0; i < arrboard.length; i++) {
            if (i == 0) {
                evidence = String.valueOf(number);
            } else {
                evidence = evidence.concat(", " + number);
            }
            if (starter == 1) {
                number = number + arrboard.length + 1;
            } else {
                number = number + arrboard.length - 1;
            }
        }
